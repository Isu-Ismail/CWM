...
def _get_history_commands(manager: StorageManager, cached: bool, active: bool):
    ...
    if active:
        ...
        if not session.get("isWatching"):
            console.print(
                "[bold red]Error:[/bold red] No active watch session. Run 'cwm watch start' first.")
            return [], None
        ...
        if not found_start:
            console.print(
                "[yellow]Warning: Could not locate 'cwm watch start' in recent history.[/yellow]")
            console.print("Showing last 50 commands instead.")
            collected = collected[-50:]

        console.print(
            f"[dim]Showing active session: {len(collected)} commands.[/dim]")
...
def _filter_and_display(commands: list, count: str, exclude: str, filter: str, list_only: bool):
    ...
    if not (filter and "cwm" in filter):
        base_list = [item for item in unique_commands if not is_cwm_call(
            item.get("cmd", ""))]
    else:
    ...
    if total_found == 0 and (exclude or filter):
        console.print(
            f"[yellow]No history found matching filters. Defaulting...[/yellow]")
    ...
    if count.lower() != "all":
        try:
            ...
            console.print(
                f"[red]Invalid count '{count}'. Defaulting to 10.[/red]")
    ...
    console.print(
        f"[bold underline]History ({len(commands_to_display)}/{total_found})[/bold underline]")
    ...
    for i, item in enumerate(commands_to_display):
        ...
        if list_id is not None:
            console.print(
                f" [cyan][{display_num}][/cyan] [dim](ID:[/dim] [green]{list_id}[/green][dim])[/dim] {cmd_text}")
        else:
...
def _filter_and_display_saved(commands: list, count: str, exclude: str, filter: str, tag: str, skip_prompt: bool = False):
    ...
    console.print(
        f"[bold underline]Saved Commands ({len(commands_to_display)}/{total_found})[/bold underline]")
    ...
    for i, item in enumerate(commands_to_display):
        ...
        console.print(
            f" [cyan][{display_num}][/cyan] [dim](ID:[/dim] [green]{sid}[/green][dim])[/dim] {fav}{var_str}[dim]--[/dim] {cmd}")
...
def get_cmd(name_or_id, id_flag, show_flag, list_mode, tag_flag,
    ...
    if hist_flag or cached_flag or active_flag:
        if id_flag or name_or_id or tag_flag or show_flag:
            console.print(
                "[red]Error: --hist cannot be used with saved command flags.[/red]")
            return
        if cached_flag and active_flag:
            console.print(
                "[red]Error: --cached and --active flags conflict.[/red]")
            return

        commands_list, total_lines = _get_history_commands(
            manager, cached_flag, active_flag)
        _filter_and_display(commands_list, count, exclude,
                            filter, list_only=list_mode)
        return

    if list_mode or tag_flag:
        if name_or_id or id_flag:
            console.print(
                "[red]Error: -l or -t cannot be used with var_name or --id.[/red]")
            return
        data_obj = manager.load_saved_cmds()
        commands = data_obj.get("commands", [])
        _filter_and_display_saved(
            commands, count, exclude, filter, tag_flag, skip_prompt=show_flag)
        return
    ...
    if id_flag is not None:
        ...
        _filter_and_display_saved(
            commands, "10", None, None, None, skip_prompt=show_flag)
        return

    if not command_to_get:
        console.print(
            f"[red]Error: '{name_or_id or id_flag}' not found.[/red]")
        return
