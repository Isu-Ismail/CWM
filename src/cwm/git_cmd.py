...
def add_account():
    ...
    alias = Prompt.ask("  [cyan]Enter unique alias[/cyan] [dim](e.g. 'work', 'personal')[/dim]")
    ...
    email = Prompt.ask(f"  [cyan]Enter email for[/cyan] [bold white]{alias}[/bold white]")
    
    # Removed spinner as requested to prevent conflict with input prompts
    ...
    try:
        ...
        if pub_key_path.exists():
            ...
            console.print(Panel(pub_key, title="Public Key", border_style="dim"))
            
            # Now safe to ask because no spinner is active
            if Confirm.ask("  [cyan]? Copy public key to clipboard?[/cyan]", default=True):
                pyperclip.copy(pub_key)
                console.print("  [green]✔ Copied![/green] Go to GitHub -> Settings -> SSH Keys and add it.")
        else:
...
def list_accounts():
    ...
    for i, acc in enumerate(accounts):
        console.print(f"  [cyan]{i+1}.[/cyan] [bold white]{acc['alias']}[/bold white]")
...
def setup_repo():
    ...
    accounts = get_configured_accounts()
    if not accounts:
        console.print("  [red]✖ No accounts found. Run 'cwm git add' first.[/red]")
        return
        
    console.print("[bold cyan]? Select Account:[/bold cyan]")
    for i, acc in enumerate(accounts):
        console.print(f"  [cyan]{i+1})[/cyan] {acc['alias']} [dim]({acc['host']})[/dim]")

    choices = [str(x) for x in range(1, len(accounts) + 1)]
    choice = IntPrompt.ask("  [dim]Enter number[/dim]", choices=choices, show_choices=False)
    ...
    with console.status("[bold cyan]Configuring local repo settings...[/bold cyan]") as status:
        ...
        console.print(f"  [green]✔ Line endings configured ({crlf_setting}).[/green]")
    ...
    if has_data:
        ...
        if Confirm.ask(f"  [cyan]? Push to remote now?[/cyan] [dim]({push_cmd})[/dim]", default=True):
            with console.status("[bold cyan]Pushing code to GitHub...[/bold cyan]"):
                success = run_git_command(["push", "-u", "origin", current_branch])
            
            if success:
                console.print("  [bold green]✔ Success! Project is live.[/bold green]")
            else:
                console.print("  [bold red]✖ Push failed.[/bold red] Check SSH key permissions.")
        ...
        if Confirm.ask("  [cyan]? Add, Commit, and Push all files?[/cyan]", default=True):
            msg = Prompt.ask("  [dim]Commit message[/dim]", default="Initial commit")
